...
async function uploadToMinIO(filePath, remotePath) {
  ...
  console.log(`📊 Archivo a subir: ${(fileSize / 1024 / 1024).toFixed(2)} MB`);
  ...
  const options = {
    partSize: 1024 * 1024 * 100, // 100MB por parte (más grande para archivos grandes)
    queueSize: 3, // Menos partes simultáneas para evitar sobrecarga
...
/**
 * Copia un archivo directamente a MinIO sin transcodificación
 * @param {string} sourcePath - Ruta local del archivo fuente
 * @param {string} remotePath - Ruta remota en MinIO
 * @returns {Promise<void>}
 */
async function copyFileToMinIO(sourcePath, remotePath) {
  try {
    console.log(`\x1b[36mCopiando archivo directamente: ${sourcePath} -> ${remotePath}\x1b[0m`);
    await uploadToMinIO(sourcePath, remotePath);
    console.log(`\x1b[32mArchivo copiado exitosamente: ${remotePath}\x1b[0m`);
  } catch (error) {
    console.error(`\x1b[31mError copiando archivo: ${error.message}\x1b[0m`);
    throw error;
  }
}
