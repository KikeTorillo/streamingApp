...
const Input = forwardRef((props, ref) => {
  ...
  // Helper functions para obtener las clases width correctas
  const getWidthClassName = (width) => {
    const fixedSizes = ['xs', 'sm', 'md', 'lg', 'xl'];
    const semanticSizes = ['auto', 'full', 'fit-content', 'min-content', 'max-content'];
    
    if (fixedSizes.includes(width)) {
      return `input-base--w-${width}`;
    } else if (semanticSizes.includes(width)) {
      return `input-base--${width === 'full' ? 'full-width' : width}`;
    }
    return null;
  };

  const getWrapperWidthClassName = (width) => {
    const fixedSizes = ['xs', 'sm', 'md', 'lg', 'xl'];
    const semanticSizes = ['auto', 'full', 'fit-content', 'min-content', 'max-content'];
    
    if (fixedSizes.includes(width)) {
      return `input-wrapper--w-${width}`;
    } else if (semanticSizes.includes(width)) {
      return `input-wrapper--${width === 'full' ? 'full-width' : width}`;
    }
    return null;
  };
  ...
  // ✅ GENERAR CLASES CSS - Solo clases que existen en CSS
  const inputClasses = [
    'input-base',
    `input-base--${size}`,
    `input-base--${variant}`,
    rounded !== 'md' && `input-base--rounded-${rounded}`,
    // ✅ MEJORADO: Soporte para tamaños fijos estándar + valores semánticos
    width && getWidthClassName(width),
    hasLeftIcon && 'input-base--with-left-icon',
    hasRightIcon && 'input-base--with-right-icon',
    !needsWrapper ? props.className : ''
  ].filter(Boolean).join(' ');

  // Clases para el wrapper - Lógica mejorada con tamaños fijos
  const wrapperClasses = [
    ...
    // ✅ MEJORADO: Soporte para tamaños fijos estándar + valores semánticos
    width && getWrapperWidthClassName(width),
