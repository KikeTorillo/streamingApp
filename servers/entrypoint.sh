...
envsubst '${NGINX_TRANSCODER1} ${NGINX_TRANSCODER2} ${NGINX_MINIO_SERVER} ${NGINX_MINIO_ACCESS_KEY} ${NGINX_SECRET_KEY} ${NGINX_REGION} ${NGINX_MINIO_BUCKET} ${NGINX_MINIO_VIDEO_DIR} ${NGINX_MINIO_COVERS_DIR} ${NGINX_MINIO_SUBS_DIR}' < /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf
